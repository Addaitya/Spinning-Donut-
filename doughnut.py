...
def render_frame(A:float, B:float):
    output = [[' ' for _ in range(screen_h)] for _ in range(screen_w)]
    z_buffer = [[0 for _ in range(screen_h)] for _ in range(screen_w)]
    cosA = cos(A)
    sinA = sin(A)
    cosB = cos(B)
    sinB = sin(B)
    ...
    while theta < 2*pi:
        ...
        while phi < 2 * pi:
            ...
            y_px = int(screen_h // 2 + y*k1*ooz)

            L = cosphi*costheta*sinB - cosA*costheta*sinphi - sinA*sintheta + cosB*(cosA*sintheta - costheta*sinA*sinphi)

            if L > 0:

                if ( 0 <= x_px < screen_w and 0 <= y_px < screen_h) and ooz > z_buffer[x_px][y_px]:
                    z_buffer[x_px][y_px] = ooz

                    l_idx = int(L*8)

                    output[x_px][y_px] = ".,-~:;=!*#$@"[l_idx]
            phi += phi_step
        theta += theta_step
